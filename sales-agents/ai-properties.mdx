...
| **Enum** | One value from a set you define | "Primary industry vertical" → FinTech, HealthTech, DevTools, Other |
...
3. Fill in:
   ...
   * **Output type** — Select Yes/No, Enum, Score, or Freeform.
   * For **Enum**: Define the list of allowed values.
...
Each property has a refresh policy that controls how often answers are re-evaluated:

* **Daily** — Re-research every day. Use for fast-changing signals (e.g., "Is this company actively hiring?").
* **Weekly** — Re-research once per week. Use for moderately dynamic signals (e.g., "Has this company raised funding recently?").

When a property value is still fresh according to its refresh policy, re-research is skipped to avoid unnecessary work.
...
4. Select the **frequency**: Daily or Weekly (weekly runs on Mondays).
...
* Companies whose existing answer is stale (based on the refresh policy) are re-researched.
...
* **Enum properties** — Breakdown by each enum value.
...
* Each research operation consumes agent credits. See [Agent Credits](/sales-agents/agent-credits) for details.

## FAQ

<AccordionGroup>
**How long does research take per company?** A few seconds per company. For large sales views with hundreds of companies, a full research run may take several minutes.

**Can I edit a property after it has been researched?** Yes. Editing instructions or output type creates a new version of the property. Existing answers are preserved until the next research cycle refreshes them.

**What happens if I change the output type?** Previous answers are kept as-is. New research produces answers in the updated format. The next auto research cycle or manual trigger re-evaluates all companies with the new output type.

**Why are some companies skipped during auto research?** Companies are skipped when their existing answer is still fresh according to the refresh policy. If you need to force a full re-evaluation, use the manual trigger with the resync option.

**Can I use AI Properties without auto research?** Yes. Properties are researched individually when company context is generated (either manually or by the overnight worker). Auto research is optional — it pre-computes answers across your sales views so they are ready before anyone visits the company page.

**How do I know if my property instructions are good?** Check the value distribution after a research run. If answers cluster heavily on one value (e.g., 90%+ "Yes"), the question may be too broad. If descriptions are vague or citations are irrelevant, the instructions need more specificity.
</AccordionGroup>