...
<AccordionGroup>
**How many products should I create?** One per distinct offering your team sells. If you sell a single platform, one product is sufficient. If you have multiple product lines with different positioning (e.g., an analytics product and an automation product), create one for each.

**Can I use the same product across multiple motions?** Yes. A product can be assigned to any number of motions.

**Does the product description appear in outreach messages?** Not directly. The AI uses the description as context when drafting emails and messages. The description influences tone and positioning but is not copied verbatim into task content.
</AccordionGroup>